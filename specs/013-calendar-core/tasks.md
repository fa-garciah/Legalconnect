...
- [ ] T001 Unit test `backend/tests/unit/calendar-input.test.ts`: the write body — required type and
      title, trimmed lengths, all-day vs timed shape, end ≥ start, the five reminder values, unknown
      fields ignored. **See it fail.**
- [ ] T002 `backend/src/modules/calendar/calendar-input.ts`.
- [ ] T003 Unit test `backend/tests/unit/calendar-audit-actions.test.ts` (three actions, target
      `calendar_event`, none channel-gated) and bump the pinned vocabulary counts (55 → 58).
      **See it fail.**
- [ ] T004 Migration `0046_calendar_event.sql` (table, enums, CHECKs, RLS, grants, indexes, audit
      CHECK) and `TENANT_SCOPED_TABLES`; `AUDIT_ACTIONS`; run `test:rls`.
- [ ] T005 Capability rows 44–45 in `matrix.ts` and `capability.ts`; update
      `capability-declared-everywhere.test.ts`'s count only once routes exist (T008).

## Phase 2: Backend routes

- [ ] T006 Isolation test `backend/tests/integration/isolation/calendar-isolation.test.ts`: firm B
...
- [ ] T007 Contract test `backend/tests/contract/calendar-events.test.ts`: list by range (overlap,
...
- [ ] T008 `calendar.repository.ts`, `calendar.service.ts`, `calendar.controller.ts`,
      `calendar.module.ts`, registration in `AppModule`.
- [ ] T009 Full backend gates: lint, typecheck, `check:env`, `npm test -- --coverage`, isolation /
      rls / role.

## Phase 3: Frontend

- [ ] T010 [P] Unit tests `frontend/tests/unit/calendar/month-grid.test.ts` (weeks start Monday; days
...
- [ ] T011 `frontend/src/calendar/{types,api,month-grid,format,schema}.ts`; capability mirror rows
      44–45 and the sync fixture.
- [ ] T012 [P] Component test `CalendarView.test.tsx`: month grid with events, today marked, day
...
- [ ] T013 [P] Component test `EventDialog.test.tsx`: create timed and all-day, case offered from
      the caller's cases, end-before-start refused before sending, edit sends only changed fields,
      cancel confirms. **See it fail.**
- [ ] T014 [P] Component test `RemindersPanel.test.tsx` and the navigation badge. **See it fail.**
- [ ] T015 Implement `/calendario` (page, view, grid, day list, dialog, reminders) and flip the
      `calendario` navigation item for MP/AA/PL/CM/SA.
- [ ] T016 Spanish-copy and wire-vocabulary (`hearing`, `deadline`, `meeting`, `scheduled`,
      `cancelled`) additions.

## Phase 4: Polish

- [ ] T017 e2e `frontend/tests/e2e/calendario.spec.ts`: create a hearing linked to a case → it shows
      on its day → edit its time → cancel it; an all-day deadline with a reminder shows in
      "Recordatorios".
- [ ] T018 Frontend gates (test, typecheck, lint, build) and colour literals.
- [ ] T019 `quickstart.md`, `quickstart-results.md`; catalog amendment (US02/US03-EP05 → MVP by 013);
      `registro-specs-mvp.md` row for 013.

## Summary

- **Total**: 19 tasks · **Done**: 0
