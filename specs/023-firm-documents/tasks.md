...
- [x] T001 Unit test — extend `backend/tests/unit/matrix-exhaustive.test.ts`'s independently
...
- [x] T002 `capability.ts`: `'document.read_list': { scope: 'tenant' }`, beside the other
...
- [x] T003 Frontend mirror: add the row to `frontend/src/authz/capability-matrix.ts` **and** to
...
- [x] T004 [P] Unit test `backend/tests/unit/like-escape.test.ts`: `escapeLike` escapes `\`, `%`
      ...
      **See it fail.**
- [x] T005 [P] `backend/src/modules/documents/like-escape.ts` — `escapeLike`, used with an
      explicit `ESCAPE '\'` in the SQL.
- [x] T006 Contract test `backend/tests/contract/firm-documents-list.test.ts`: `GET
...
- [x] T007 `DocumentsRepository.listForTenant` — the join to `case_file`, the single predicate
      list shared by the page query and `count(*)`, the `(uploaded_at, id)` cursor via `toPage`.
- [x] T008 `FirmDocumentsController` at `@Controller('tenant/documents')`:
...
- [x] T009 **Landed in `firm-documents-filters.test.ts`** rather than its own file: it needs the identical fixture, and building it twice is two chances for the two tests to disagree about what they measure. The leak test.
...
- [x] T009a **FR-002 — minimisation.** In `firm-documents-list.test.ts`, assert the response
...
- [x] T009b **FR-016, Decision 6 — the list is not audited.** In the same file, count
...
- [x] T010 Contract test `backend/tests/contract/firm-documents-filters.test.ts`: `categoryId` and
...
- [x] T011 Contract test `backend/tests/contract/firm-documents-route.test.ts`: using the
...
- [x] T012 `listFirmDocuments(query)` in `frontend/src/app/documents/api.ts` plus its types,
      omitting absent filters from the query string the way `cases/api.ts`'s `meaningful()` does.
- [x] T013 [P] Unit test `frontend/tests/unit/view-mode.test.ts`: `readViewMode` defaults to grid
...
- [x] T014 [P] `frontend/src/documents/view-mode.ts`.
- [x] T015 Component test `frontend/tests/component/firm-documents.test.tsx`: the heading, the
...
- [x] T016 `frontend/src/app/documentos/page.tsx` (server, resolving archetype exactly as
      `/clientes` does) and `FirmDocuments.tsx`.
- [x] T017 `DocumentCard.tsx` and `DocumentRow.tsx` — file name as the title (Decision 5), the
      full name as the accessible name when truncated.
- [x] T018 Component test for `DocumentFilters.tsx`: search debounces at 300 ms; the category and
      matter selects use the `ALL` sentinel and are not debounced; "Limpiar filtros" clears all
      three. **See it fail**, then build it.
- [x] T019 `ViewModeToggle.tsx`, persisting per viewer (FR-011).
- [x] T020 Component test: preview and download from a card call `021`'s modules with **that
...
- [x] T021 Component test `frontend/tests/component/upload-from-firm.test.tsx`: the dialog asks
...
- [x] T022 `UploadFromFirmDialog.tsx`, wrapping `021`'s `UploadDialog` beneath the matter step.

## Phase 5: Navigation and copy

- [x] T023 Unit test — extend `frontend/tests/unit/navigation-items.test.ts`: `documentos` is
...
- [x] T024 Flip the flag and narrow the archetypes — both in one edit, because either alone is
      wrong.
- [x] T025 Extend `frontend/tests/component/spanish-copy.test.tsx` with an `it(...)` rendering the
...
- [x] T026 Grep the new files for colour literals; expect zero (`020`/FR-009).

## Phase 6: Gates and closure

- [x] T027 Backend gates (full `npm test` and the coverage run: see quickstart-results.md — not completed here): `typecheck`, `lint`, `test`, and with a database `test:rls`,
...
- [x] T029 e2e `frontend/tests/e2e/firm-documents.spec.ts` — sign in as `022`'s `MP`, open
...
- [x] T030 `specs/023-firm-documents/quickstart.md` — how to see the screen against `022`'s demo
      firm, as `MP` and as an `AA`, and what differs between them.
- [x] T031 `specs/023-firm-documents/quickstart-results.md` — verified and not verified, by name.
- [x] T032 Leave the spec's eight Decision boxes **unticked** (pending Jero) and confirm the
