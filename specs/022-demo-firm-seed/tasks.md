...
- [ ] T001 [US4] Unit test `backend/tests/unit/demo-guard.test.ts`: `assertLocalDemoTarget`
...
- [ ] T002 [US4] `backend/drizzle/demo/guard.ts` — `DemoSeedRefused` and
...
- [ ] T003 [P] Unit test `backend/tests/unit/demo-rng.test.ts`: `mulberry32(SEED)` yields the same
...
- [ ] T004 [P] `backend/drizzle/demo/rng.ts` — `mulberry32`, `pick`, `shuffle`, `intBetween`, one
      exported `DEMO_SEED`.
- [ ] T005 [P] Unit test `backend/tests/unit/demo-deterministic-id.test.ts`: `deterministicUuid`
...
- [ ] T006 [P] `backend/drizzle/demo/deterministic-id.ts` — SHA-256 → RFC-4122-shaped UUID.
- [ ] T007 `backend/drizzle/load-env.ts` — the single `loadEnvFile` (FR-015); delete the copies in
...
- [ ] T007a **Principle VI — keep the fixture credentials out of the shipped artifact.** Unit test
...
- [ ] T008 `backend/src/common/storage/object-store/object-store.config.ts` —
...
- [ ] T009 Unit test `backend/tests/unit/demo-firm-data.test.ts`: exactly seven people; their
...
- [ ] T009a **FR-019 — the demo firm is Spanish.** Extend `demo-firm-data.test.ts` (or a sibling
...
- [ ] T010 `backend/drizzle/demo/firm.ts` — the two firms, the seven people, the three demo-owned
...
- [ ] T011 Unit test `backend/tests/unit/demo-totp-usable.test.ts`: a code generated by
...
- [ ] T012 Integration test `backend/tests/integration/demo-seed-auth-rows.test.ts`: after the
...
- [ ] T013 `backend/drizzle/seed-demo.ts` — entry point: guard, env, connections, then firms →
      plans → people → auth rows → positions → directory entries, all idempotent. Prints the
      credential table.
- [ ] T014 Contract test `backend/tests/contract/demo-sign-in.test.ts`: for **each of the seven**
...
- [ ] T014a **FR-020 — the printed output.** Unit test
...
- [ ] T015 [P] Unit test `backend/tests/unit/demo-clients.test.ts`: 25 names for firm 1 and 4 for
...
- [ ] T016 [P] `backend/drizzle/demo/clients.ts`.
- [ ] T017 Unit test `backend/tests/unit/demo-matters.test.ts`: 40 matters for firm 1; file
...
- [ ] T018 `backend/drizzle/demo/matters.ts`.
- [ ] T019 Integration test `backend/tests/integration/demo-seed-matters.test.ts`: after the
...
- [ ] T020 Wire clients and matters into `seed-demo.ts`.

## Phase 5: Documents with real bytes (US3)

- [ ] T021 [P] Unit test `backend/tests/unit/demo-file-bytes.test.ts`: `minimalPdf(seedText)`
...
- [ ] T022 [P] `backend/drizzle/demo/documents.ts` — generation plus the two byte builders.
- [ ] T023 Unit test `backend/tests/unit/demo-documents.test.ts`: ~130 documents, unevenly
      ...
      length. **See it fail.**
- [ ] T024 Integration test `backend/tests/integration/demo-seed-storage-counter.test.ts`: after
      ...
      **See it fail.**
- [ ] T025 Integration test `backend/tests/integration/demo-seed-objects.test.ts` *(needs MinIO)*:
...
- [ ] T026 Wire documents into `seed-demo.ts`, including the FR-016 degraded path: rows always
      written, unreachable-store reported per document, exit code non-zero.
- [ ] T027 Integration test `backend/tests/integration/demo-seed-store-unreachable.test.ts`: with
...
- [ ] T028 [P] Unit test `backend/tests/unit/demo-calendar.test.ts`: ~18 events; more than one
...
- [ ] T029 [P] `backend/drizzle/demo/calendar.ts`, wired into `seed-demo.ts`.

## Phase 7: Idempotency, determinism and the script entry

- [ ] T030 Integration test `backend/tests/integration/demo-seed-idempotent.test.ts`: run the
...
- [ ] T031 `package.json` — `"db:seed:demo": "tsx drizzle/seed-demo.ts"`. Also confirm
      `npm run check:env` still passes (no new environment variable is introduced).
- [ ] T032 Unit test `backend/tests/unit/demo-no-mfa-bypass.test.ts`: reading every file under
...
- [ ] T032a **SC-009 — secret scanning.** Read the repository's `gitleaks` configuration
...
- [ ] T033 Backend gates: `npm run typecheck`, `npm run lint`, `npm test`, and with a database:
...
- [ ] T034 Frontend gates: `npm run typecheck`, `npm run lint`, `npm test`, `npm run build`. This
      slice changes no frontend file, so these are a regression check, not a verification.
- [ ] T035 `specs/022-demo-firm-seed/quickstart.md` — the setup walkthrough, the full credential
      table (emails, shared password, per-person TOTP secret and backup codes) and the guard's
      two refusals, written by hand (FR-018).
- [ ] T036 `specs/022-demo-firm-seed/quickstart-results.md` — what was verified and what was not,
      naming every suite that did not run and why. Never claim a suite that did not run.
- [ ] T037 Re-read the spec's Approval Checklist; leave the nine Decision boxes **unticked**
